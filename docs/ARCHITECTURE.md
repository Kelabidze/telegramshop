...
216 тестов, чистый `node:test` через `tsx`, без Jest/Vitest.
...
| `apps/api/src/variations.test.ts`         | 11 тестов вариаций: карточка одна, агрегаты, фильтр по стране, родителя нельзя купить |
