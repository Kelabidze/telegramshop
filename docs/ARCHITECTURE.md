...
apps/miniapp/src/
  ...
  assets/index.ts            единственная точка входа в брендовую графику:
                             emptyArt, brand, decor, categoryIcon(); Vite хеширует
                             имена, поэтому переименование — ошибка сборки, не 404
  assets/                    illustrations/ logo/ decorations/ icons/ fonts/,
                             в каждой README с требованиями и решениями
  components/CategoryGlyph.tsx  иконка категории CSS-маской — <img> не наследует цвет
...
### 3.3. Резерв склада: почему только для крипто

`LicenseKey` получил `reservedUntil` + `reservedForLineId`. Асимметрия
намеренная:

| | Stars | USDT |
| --- | --- | --- |
| окно между заказом и оплатой | секунды | минуты |
| обратимость | оплата не началась | перевод необратим |
| цена проигранной гонки | `OUT_OF_STOCK` до оплаты | деньги ушли, заказ `FAILED` |

Поэтому резерв ставится только при создании крипто-платежа: для Stars он ничего
не спасает и лишь замедлял бы обычный путь.

Три свойства, которые важнее самого механизма:

1. **Резерв advisory.** Он не заменяет условный `UPDATE` в `claimLicenseKey` —
   тот по-прежнему решает, чей ключ. Даже если два резерва пересекутся,
   перепродажи не будет. Резерв лишь не даёт **более позднему** оформлению
   рассчитывать на ключ, которого ждёт **платящий** покупатель.
2. **Срок, а не флаг.** Истёкший резерв автоматически снова считается доступным.
   Никакому коду не нужно помнить, что резерв надо снять, — а именно это было бы
   способом отказа у булева флага. Отмена и истечение платежа снимают резерв
   сразу, но это оптимизация, а не условие корректности.
3. **Свой резерв доступен своему заказу.** `claimLicenseKey` сначала ищет ключ,
   зарезервированный для этой строки, и только потом свободный. Обратный порядок
   позволил бы забрать ключ, за который уже летят чужие деньги.

Следствие для витрины: «осталось N» может уменьшиться на время открытого
оформления и вернуться, если его бросили. Это честная цифра — она означает то,
что реально может купить новый покупатель, а больше ни для чего она не нужна.
...
- **Корень слота «Каталог» зависит от режима**, и это единственное расхождение
  между режимами: у покупателя слот начинается с `home` (главная), полный список
  лежит на шаг глубже; у режима управления `home` нет вообще. Знание об этом
  собрано в двух функциях `App.tsx`, а не размазано по проверкам:
  `rootViewForTab()` отвечает, к какому экрану сбрасывает вкладка, а
  `resolveView()` отображает `home` на слот каталога, когда рендерит режим
  управления. Без второй функции админ получал пустую контентную область: режим
  определяется по `/api/me`, то есть **после** первого рендера, поэтому стек уже
  содержит `home`, которого ни одна админская ветка не знает.
...
- **Иконки категорий — брендовые SVG, а не эмодзи из БД.** Резолвит
  `categoryIcon()` (`assets/index.ts`): точная карта слагов, затем сеть по ключевым
  словам, затем `other.svg`. Именно двухуровневость обязательна — категории
  редактируются в админке, поэтому точная карта гарантированно устаревает: живой
  каталог уже пришёл со слагами, которых карта не знала, и все плитки молча падали
  в эмодзи. Порядок ключевых слов — правило: `giftcards` содержит `cards`, а
  «Подарочные карты» содержит «карт». Эмодзи остаётся полем категории и живёт в
  админке, но витрина рисует не его. Флаги стран по-прежнему эмодзи.
- **Иконка категории — CSS-маска** (`components/CategoryGlyph.tsx`), не `<img>`.
  Файлы пака обведены `#E83DFF`, а в `<img>` `currentColor` не достаёт: картинкой
  все глифы сидят на полном акценте, и выбранное состояние выразить нечем. Маска
  берёт альфу, цвет даёт CSS. `-webkit-mask` рядом с `mask` обязателен.
- **Декор — только `--zone-decor-tentacle`, и только в двух местах**: за hero
  профиля и в карточке «Сейчас в ZONE», оба раза `contain` и под 0.2 прозрачности.
  Значение выставляется из `main.tsx`, чтобы URL пришёл из Vite-импорта, и
  **обязательно в кавычках**: инлайненный data-URI содержит апострофы и вложенный
  `url(%23g)`, поэтому без кавычек это невалидный CSS, а `setProperty` на
  непарсящемся значении молча оставляет прежнее — то есть `none`. Так декор не
  рисовался вообще, без единого следа в консоли; закреплено тестом в
  `bundle.test.ts`. `grid`, `noise`, `glow` и `suction-row` намеренно не
  подключены: причины — в `assets/decorations/README.md`.
- **`EmptyState` принимает `art` **или** `emoji`.** Пять иллюстраций пака покрывают
  все покупательские состояния; три админских экрана остаются на эмодзи —
  рисовать шестую иллюстрацию туда, куда покупатель не попадает, незачем.
- **Словесный знак — CSS-текст, а не `wordmark.svg`.** Файл пака набирает буквы
  через `<text font-family="Arial Black">`, а SVG в `<img>` — изолированный
  документ, шрифтов приложения он не видит; Arial Black нет ни на Android, ни на
  iOS. Название магазина — единственная строка, которой нельзя выглядеть
  случайной, поэтому её ставит `.brand-signature__wordmark`.
- `--zone-font-display` указывает на системный стек; требования к файлу — в
...
| Брендовый ассет (лого, иллюстрация)| README в `src/assets/<папка>/` → экспорт в `assets/index.ts` → импорт в компоненте или `--zone-decor-*` (значение из `main.tsx`, **в кавычках**) |
| Иконка категории                   | `categoryIcon()` в `src/assets/index.ts` → `components/CategoryGlyph.tsx` |
