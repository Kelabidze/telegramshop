...
193 теста, чистый `node:test` через `tsx`, без Jest/Vitest.
...
| `apps/api/src/routes/admin.test.ts`       | 90 тестов управления: защита каждого роута, CRUD, баннеры, загрузка медиа |
