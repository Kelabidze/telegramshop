...
// Rapira off: these suites assert exact USDT amounts, so the rate must be fixed
// and the tests must not depend on a live exchange being reachable. The live-rate
// path has its own suite in rapira-rates.test.ts.
