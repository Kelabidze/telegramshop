...
# Demo banners were seeded on the first deploy and are now managed through the
# admin panel. Automatic seeding is disabled to prevent re-creating deleted
# banners on every deploy.
#
# To manually seed demo banners on a fresh database, run:
#   node --env-file=/srv/shop/shared/api.env apps/api/dist/cli/seed-banners.js
#
# Or in dev:
